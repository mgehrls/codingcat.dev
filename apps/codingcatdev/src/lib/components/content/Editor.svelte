<script lang="ts">
	...
	const models: Map<string, editor.ITextModel> = new Map();
	...
	onMount(() => {
		...
		if (dev && !/chrome/i.test(navigator.userAgent)) {
			console.debug('Editor: addedInnerHTML');
			container.innerHTML =
	 ...
	 */
	function init(monaco: any, dark_mode: boolean) {
	...
	$: if (instance && true /* to retrigger on stubs change */) {
		// @ts-ignore
		const model = models.get('/src/lib/App.svelte');
